			...
			client.post(mContext, Constants.LOGIN_URL, entity,
					"application/json", new AsyncHttpResponseHandler() {
						...
						public void onSuccess(int arg0, Header[] arg1,
							...
							if (loginInfo != null
							...
							} else if (loginInfo.getErrorCode() == Constants.ERROR_TOKEN_INVALID) {
								mContext.startService(new Intent(mContext,
										AutoLoginService.class));
							} else if (loginInfo.getErrorCode() == Constants.ERROR_USERNAME_PASSWORD_SAME) {
								loading.dismiss();
								...
								Toast.makeText(mContext, "无效的邮箱账号",
										Toast.LENGTH_SHORT).show();
							} else if (loginInfo.getErrorCode() == Constants.ERROR_PASSWORD_INVALID) {
								loading.dismiss();
