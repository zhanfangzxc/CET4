...
public class DownloadAdapter extends BaseAdapter {
	...
	public DownloadAdapter(Context context, ArrayList<Practice> practices,
		...
		for (int i = 0; i < practices.size(); i++) {
			...
			sb.append("·" + "第" + (i + 1) + "题.mp3");
			practices.get(i).setFileName(sb.toString());
		}
	}
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		String fileName = practice.getFileName();
		holder.name.setText(fileName.substring(fileName.lastIndexOf("·"),
				fileName.lastIndexOf(".")));
		File dir = new File(Environment.getExternalStorageDirectory(), "cet6");
		if (!dir.exists()) {
			dir.mkdir();
		}
		final File file = new File(dir, practice.getFileName());
		if (file.exists()) {
	...
	private void startDownload(String url, int position, String fileName) {
		...
		request.setDestinationInExternalPublicDir("cet6", fileName);
